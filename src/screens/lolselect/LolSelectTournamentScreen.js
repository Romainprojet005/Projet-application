...
import {
  View, Text, StyleSheet, TouchableOpacity, Platform, Animated, Image,
} from 'react-native';
...
export default function LolSelectTournamentScreen({ route, navigation }) {
  ...
  const [result]    = useState(() => simulateMatch(team, rivalTeam));
      ...
      <LinearGradient colors={OB_BG} style={styles.container}>
        <PageScroll contentContainerStyle={styles.scroll}>
          <Animated.View style={{ opacity: fadeAnim }}>
            ...
            <Text style={styles.vsSub}>Votre équipe affronte une sélection rivale tirée des joueurs restants</Text>

            <View style={styles.rosterRow2}>
              <RosterColumn title="🛡️ VOTRE ÉQUIPE" team={team} highlight />
              <View style={styles.vsBadge}><Text style={styles.vsBadgeText}>VS</Text></View>
              <RosterColumn title="🔥 ÉQUIPE ADVERSE" team={rivalTeam} />
            </View>
    ...
    <LinearGradient colors={OB_BG} style={styles.container}>
      <PageScroll contentContainerStyle={styles.scroll}>
        <Animated.View style={[styles.finalCard, { opacity: fadeAnim }]}>
          <Text style={styles.finalEmoji}>{userWon ? '🏆' : '💔'}</Text>
          <Text style={styles.finalTitle}>{userWon ? 'VICTOIRE !' : 'DÉFAITE'}</Text>
          <Text style={styles.finalSub}>
            {userWon ? 'Votre équipe remporte le tournoi' : "L'équipe adverse remporte le tournoi"}
          </Text>

          <View style={styles.scoreRow}>
            <View style={styles.scoreCell}>
              <Text style={styles.scoreLabel}>VOTRE ÉQUIPE</Text>
              <Text style={[styles.scoreValue, userWon && { color: ACCENT_LIGHT }]}>{result.userStats.total.toFixed(1)}</Text>
              <Text style={styles.scoreSub}>{result.laneWinsUser} duel{result.laneWinsUser > 1 ? 's' : ''} gagné{result.laneWinsUser > 1 ? 's' : ''}</Text>
            </View>
            <Text style={styles.scoreSep}>—</Text>
            <View style={styles.scoreCell}>
              <Text style={styles.scoreLabel}>ADVERSAIRE</Text>
              <Text style={[styles.scoreValue, !userWon && { color: HEXTECH }]}>{result.rivalStats.total.toFixed(1)}</Text>
              <Text style={styles.scoreSub}>{result.laneWinsRival} duel{result.laneWinsRival > 1 ? 's' : ''} gagné{result.laneWinsRival > 1 ? 's' : ''}</Text>
            </View>
          </View>
          ...
          <TouchableOpacity onPress={() => navigation.replace('LolSelectDraft')} style={styles.replayBtn} activeOpacity={0.8}>
            <Text style={styles.replayBtnText}>🔄  Nouveau draft</Text>
          </TouchableOpacity>
        </Animated.View>
      </PageScroll>
    </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  finalSub: { fontSize: 13, color: colors.textSecondary, marginBottom: spacing.md },
