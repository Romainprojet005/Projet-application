...
import { QUELPLUS_CATEGORIES } from '../../data/quiEstLePlusPrompts';
...
export default function TriPotesSetupScreen({ navigation }) {
  ...
  const handleStart = () => {
    if (!canStart) return;
    navigation.navigate('TriPotesGame', { roundCount, playerNames: validPlayers, categoryId });
  };
      ...
      <PageScroll contentContainerStyle={styles.scroll}>
        ...
        <Animated.View style={[styles.header, { opacity: fadeIn, transform: [{ translateY: slideUp }] }]}>
          ...
          <View style={styles.badge}>
            ...
            <View>
              <Text style={styles.badgeName}>La Marquise</Text>
              <Text style={styles.badgeQuote}>"Le classement ne ment jamais…"</Text>
            </View>
          </View>
          <Text style={styles.pageTitle}>TRIPOTES</Text>
          <Text style={styles.pageSubtitle}>Classez vos potes, devinez la question</Text>
        </Animated.View>
        ...
        <Animated.View style={[styles.form, { opacity: fadeIn, transform: [{ translateY: slideUp }] }]}>
          ...
          <View style={[styles.rulesCard, { borderColor: ACCENT + '35', backgroundColor: ACCENT + '15' }]}>
            ...
            <Text style={styles.rulesLine}>🤫  Le juge découvre seul une question secrète</Text>
            <Text style={styles.rulesLine}>📊  Il classe les autres joueurs selon cette question</Text>
            <Text style={styles.rulesLine}>🔎  Le groupe voit le classement et doit <Text style={styles.rulesAccent}>deviner la vraie question</Text> parmi 4</Text>
            <Text style={styles.rulesLine}>🏆  Bonne pioche → le groupe marque. Bluff réussi → le juge marque !</Text>
          </View>
